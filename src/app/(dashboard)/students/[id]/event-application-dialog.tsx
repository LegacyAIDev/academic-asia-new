...
import {
  createStudentEventApplication,
  updateStudentEventApplication,
  deleteStudentEventApplication,
  type CreateEventAppInput,
...
type EventTypeItem = { id: number; code: string; label: string; color: string | null }
type EventItem = { id: string; name: string; event_type_id: number }
...
export function EventApplicationDialog({
  ...
  // Derive initial event type from the application's event
  ...
  const { eventTypes, events, statuses } = referenceData
  ...
  // Reset event type filter when dialog opens/closes
      ...
      >
          ...
          <div className="relative flex items-center gap-4 px-6 py-5">
            ...
            <DialogHeader className="flex-1 space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                {mode === "create"
                  ? "Register for an event (expo, interview, etc.)"
                  : "Update event application details"}
              </p>
            </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <FormSection icon={CalendarDays} title="Event Selection" accentColor="primary">
              <FormField label="Event Type (filter)">
                <Select
                  value={selectedEventTypeId}
                  onValueChange={(val) => setSelectedEventTypeId(val === "__all__" ? "" : val)}
                >
            ...
            <FormSection icon={ClipboardList} title="Status & Communication" accentColor="teal">
              <div className="grid grid-cols-2 gap-3">
                ...
                <FormField label="Email Sent">
                  <Select name="email_sent" defaultValue={application?.email_sent ? "true" : "false"}>
                    <SelectTrigger className={selectTriggerStyles}>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
...
export function EditEventApplicationButton({
  application,
  studentId,
  referenceData,
}: {
...
export function DeleteEventApplicationButton({
  applicationId,
  studentId,
  eventName,
}: {
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setError(result.error ?? "Failed to delete event application")
      }
    })
  }
        ...
        <AlertDialogHeader>
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-1.5">
              <AlertDialogTitle className="text-lg">Delete Event Application</AlertDialogTitle>
              <AlertDialogDescription>
                Are you sure you want to delete the event application for <strong className="text-foreground">{eventName}</strong>?
