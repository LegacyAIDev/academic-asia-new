...
export function StudentInternalNotesSection({ studentId, notes }: Props) {
  ...
  const hasAnyNote = notes.character || notes.strategy || notes.parent_expectations ||
    notes.disability_sen || notes.medical_notes || notes.guardianship_notes || notes.additional_remarks
  ...
  return (
    <Card className="border-0 shadow-sm">
      ...
      <CardContent className="space-y-4">
        {!hasAnyNote && (
          <p className="text-sm text-muted-foreground text-center py-4">All fields are empty — click Edit to add notes.</p>
        )}
