...
-- ============================================================================
-- EVENT_INTERVIEWERS (School representatives at events)
-- Consolidates: Expo_Interviewer, II_Interviewer, MA_Interviewer, TS_Interviewer
-- ============================================================================

create table public.event_interviewers (
  id uuid primary key default gen_random_uuid(),
...
create index event_interviewers_event_id_idx on public.event_interviewers(event_id);
create index event_interviewers_school_id_idx on public.event_interviewers(school_id);
create index event_interviewers_assigned_to_idx on public.event_interviewers(assigned_to);
...
alter table public.event_interviewers enable row level security;
...
create policy "event_interviewers_select" on public.event_interviewers
  for select to authenticated using (true);
...
create policy "event_interviewers_insert" on public.event_interviewers
  for insert to authenticated with check (true);
...
create policy "event_interviewers_update" on public.event_interviewers
  for update to authenticated using (true) with check (true);
...
create trigger event_interviewers_updated_at before update on public.event_interviewers
  for each row execute function public.handle_updated_at();
...
comment on table public.event_interviewers is 'School representatives/interviewers at events';
