/**
 * Data Migration Script: Import event_exams from multiple legacy CSV files
 * 
 * Consolidates: AA_Event_Exam, AA_Expo_Exam, AA_II_Exam, AA_TS_Exam
 * 
 * Usage:
 *   npx tsx scripts/migrate-event-exams.ts --dry-run
 *   npx tsx scripts/migrate-event-exams.ts
 */
...
// Source configurations (no MA - music auditions don't have entrance exams)
const SOURCES = [
  { file: 'AA_Event_Exam.csv', legacyTable: 'event', idField: 'event_id' },
  { file: 'AA_Expo_Exam.csv', legacyTable: 'expo', idField: 'expo_id' },
  { file: 'AA_II_Exam.csv', legacyTable: 'indinterview', idField: 'indinterview_id' },
  { file: 'AA_TS_Exam.csv', legacyTable: 'topschool', idField: 'topschool_id' },
];
...
function transformRow(row: Record<string, string>, source: typeof SOURCES[0]): TransformedRecord | null {
  ...
  return {
    ...
    apply_year: cleanString(row['applyyear']),
    entrance_exam_date: parseDate(row['entranceexamdate']),
...
async function migrate() {
  console.log('📝 Starting event_exams migration...\n');
  ...
  for (let i = 0; i < allRecords.length; i += BATCH_SIZE) {
    ...
    const { data: result, error } = await supabase.from('event_exams').insert(batch).select('id');
    if (error) {
